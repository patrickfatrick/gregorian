...
test('parses an ISO-8601 string with milliseconds', t => {
  t.is(
    ...
    new Date(2019, 9, 23, 5, 28, 41, 506.245).toISOString(),
  );
});
...
test('parses an ISO-8601 string with milliseconds (rounding up)', t => {
  t.is(
    ...
    new Date(2019, 9, 23, 5, 28, 41, 506.789).toISOString(),
  );
});

test('parses an ISO-8601 string with milliseconds in hundredths', t => {
  t.is(
    parse('2019-10-23T05:28:41.57').toISOString(),
    new Date(2019, 9, 23, 5, 28, 41, 570).toISOString(),
  );
});

test('parses an ISO-8601 string with less than 100 milliseconds', t => {
  t.is(
    parse('2020-04-29T18:47:45.042003').toISOString(),
    new Date(2020, 3, 29, 18, 47, 45, 42.003).toISOString(),
  );
});
