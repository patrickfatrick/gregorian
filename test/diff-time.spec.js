...
test('errors out if invalid date1 passed in', (t) => {
  t.throws(() => diffTime('y')('1988-04-11T00:00:00.000Z')(new Date()), TypeError)
})

test('errors out if invalid date2 passed in', (t) => {
  t.throws(() => diffTime('y')(new Date())('1988-04-11T00:00:00.000Z'), TypeError)
})

test('errors out if invalid increment passed in', (t) => {
  t.throws(() => diffTime('invalid')(new Date('2017-01-01'))(t.context.date1), TypeError)
})

test('uses the current time by default (date1)', (t) => {
  const clock = sinon.useFakeTimers(new Date('1985-05-22T00:00:00.000Z'))
  t.is(diffTime('d')()(new Date('1985-05-23T00:00:00.000Z')), 1)
  clock.restore()
})
...
test('returns numeric difference in years between two dates', (t) => {
  t.is(Math.round(diffTime('y')(t.context.date1)(t.context.date2)), 1)
})

test('returns numeric difference in months between two dates', (t) => {
  t.is(Math.round(diffTime('m')(t.context.date1)(t.context.date2)), 12)
})

test('returns numeric difference in weeks between two dates', (t) => {
  t.is(Math.round(diffTime('w')(t.context.date1)(t.context.date2)), 52)
})

test('returns numeric difference in days between two dates', (t) => {
  t.is(diffTime('d')(t.context.date1)(t.context.date2), 365)
})

test('returns numeric difference in hours between two dates', (t) => {
  t.is(diffTime('h')(t.context.date1)(t.context.date2), 8760)
})

test('returns numeric difference in minutes between two dates', (t) => {
  t.is(diffTime('t')(t.context.date1)(t.context.date2), 525600)
})

test('returns numeric difference in seconds between two dates', (t) => {
  t.is(diffTime('s')(t.context.date1)(t.context.date2), 31536000)
})

test('returns numeric difference in milliseconds between two dates', (t) => {
  t.is(diffTime('l')(t.context.date1)(t.context.date2), 31536000000)
})
...
test('accounts for leap days', (t) => {
  t.is(diffTime('d')(new Date('1988-02-01T00:00:00.000Z'))(new Date('1988-03-01T00:00:00.000Z')), 29)
})
